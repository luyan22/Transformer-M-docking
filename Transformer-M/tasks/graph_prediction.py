...
from ..data.docking_data import (DockingDataset, DS_Tasks)
...
class GraphPredictionTaskQM9(FairseqTask):
    ...
    @staticmethod
    def add_args(parser):
        parser.add_argument("--dataset_name", default="QM9", type=str, help="name of the dataset")
        parser.add_argument("--runseed", default=0, type=int, help="starting number of training split")
    ...
    def load_dataset(self, split, combine=False, **kwargs):
        ...
        if self.args.dataset_name in DS_Tasks:
            dataset = DockingDataset(batched_data, task_idx=self.cfg.task_idx)
        dataset = NestedDictionaryDataset({
            "nsamples": NumSamplesDataset(),
            "net_input": {
                "batched_data": batched_data
